...
gen_reports() {
  echo "[entrypoint] Generating product_service coverage HTML..."
  PYTHON=python3; command -v python3 >/dev/null 2>&1 || PYTHON=python
	"$PYTHON" - <<'PY'
import os, sys, subprocess, time
data_file = os.environ.get('COVERAGE_FILE', '/coverage/.coverage.product_service')
rcfile = '.coveragerc'
...
try:
	...
	if not used:
		print('[entrypoint] No coverage data found for product_service in', cov_dir)
	else:
...
if [ -n "${COVERAGE:-}" ]; then
	...
	export COVERAGE_FILE=${COVERAGE_FILE:-/coverage/.coverage.product_service}
